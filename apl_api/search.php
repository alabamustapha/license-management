...
$search_types_array=array("product", "client", "license");
...
if (!empty($api_key_secret) && !empty($api_function) && $api_post_key==hash("sha256", $ROOT_URL) && $submit_ok=="Submit" && $refer=="$ROOT_URL/apl_api/api.php" && in_array($user_agent, $SUPPORTED_BROWSERS_ARRAY)) //prevent someone from posting to this file directly
    ...
    if ($api_action_success==1) //everything OK
        ...
        if (isset($submit_ok))
            {
            if (!empty($search_keyword) && in_array($search_type, $search_types_array))
                {
                if ($error_detected!=1)
                    {
                    if ($search_type=="product")
                        {
                        $stmt=mysqli_prepare($GLOBALS["mysqli"], "SELECT * FROM apl_products WHERE product_title=?");
                        if ($stmt)
                            {
                    ...
                    if ($search_type=="client")
                        {
                        $stmt=mysqli_prepare($GLOBALS["mysqli"], "SELECT * FROM apl_clients WHERE client_email=?");
                        if ($stmt)
                            {
                            mysqli_stmt_bind_param($stmt, "s", $search_keyword);
                            $exec=mysqli_stmt_execute($stmt);
                    ...
                    if ($search_type=="license")
                        {
                        $stmt=mysqli_prepare($GLOBALS["mysqli"], "SELECT * FROM apl_licenses JOIN apl_products ON apl_licenses.product_id=apl_products.product_id LEFT JOIN apl_clients ON apl_licenses.client_id=apl_clients.client_id WHERE  (apl_clients.client_email=? OR apl_licenses.license_code=?)");
                        if ($stmt)
                            {
                    ...
                    if ($results_total<1) //no records found
                        {
                        $error_detected=1;
                        $error_details.="No records found.<br>";
                        }
                    else //records found
                        {
                        $action_success=1;
                ...
                {
                $error_detected=1;
                $error_details.="Invalid search type or keyword.<br>";
                }
            ...
            if ($action_success==1) //everything OK
                ...
                {
                $page_message="The results could not be displayed because of this error:<br><br>$error_details";
                }
            }
        }
    else //display error message
