...
$page_message="View existing installations. If any installation needs to be modified, click the product name. If any installation needs to be deleted, check the box near client name (or license code) and click the 'Submit' button.<br><br>For security reasons, only installation IP address can be modified.";
...
//list installations
function returnInstallationsArray()
    {
    $root_array=array();

    $results=mysqli_query($GLOBALS["mysqli"], "SELECT * FROM apl_installations JOIN apl_products ON apl_installations.product_id=apl_products.product_id LEFT JOIN apl_clients ON apl_clients.client_id=apl_installations.client_id ORDER BY installation_id DESC");
...
if (isset($submit_ok))
    {
    if (isset($installation_ids_array) && is_array($installation_ids_array)) //delete selected records
        {
...
$installations_array=returnInstallationsArray();
