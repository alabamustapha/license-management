<?php
//used to check if license is active and add license details to database during software installation
...
if (filter_var($ip_address, FILTER_VALIDATE_IP) && in_array($user_agent, $SUPPORTED_BROWSERS_ARRAY) && FILTER_VAR($product_id, FILTER_VALIDATE_INT) && filter_var($root_url, FILTER_VALIDATE_URL, FILTER_FLAG_SCHEME_REQUIRED | FILTER_FLAG_HOST_REQUIRED) && $root_url==$refer && !empty($installation_hash) && $installation_hash==hash("sha256", $root_url.$client_email.$license_code) && !empty($license_signature))
    ...
    if ($error_detected!=1) //everything OK so far, do final check if license is really active
        ...
        if ($license_status==1) //license active, add or update installation details in database
            ...
            if (filter_var($client_id, FILTER_VALIDATE_INT)) //client_id is not null (so license_code is null), search by client_id (prepared statements has limitations when using where clauses with null values)
                {
                $stmt=mysqli_prepare($GLOBALS["mysqli"], "SELECT * FROM apl_installations WHERE product_id=? AND client_id=? AND installation_ip=? AND installation_domain=?");
                if ($stmt)
                    {
                    mysqli_stmt_bind_param($stmt, "iiss", $product_id, $client_id, $ip_address, $installation_domain);
                    $exec=mysqli_stmt_execute($stmt);
                ...
                {
                $stmt=mysqli_prepare($GLOBALS["mysqli"], "SELECT * FROM apl_installations WHERE product_id=? AND license_code=? AND installation_ip=? AND installation_domain=?");
                if ($stmt)
                    {
                    mysqli_stmt_bind_param($stmt, "isss", $product_id, $license_code, $ip_address, $installation_domain);
                    $exec=mysqli_stmt_execute($stmt);
            ...
            if ($selected_installation_results_total<1) //this is a new installation
                {
                $stmt=mysqli_prepare($GLOBALS["mysqli"], "INSERT IGNORE INTO apl_installations (client_id, license_code, product_id, installation_ip, installation_domain, installation_date, installation_status, installation_hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                if ($stmt)
                    {
                    mysqli_stmt_bind_param($stmt, "isisssis", $client_id, $license_code, $product_id, $ip_address, $installation_domain, $installation_date, $installation_status, $installation_hash);
                    $exec=mysqli_stmt_execute($stmt);
                ...
                if ($added_records>0)
                    {
                    $action_success=1;
                    $report_text="New $product_title installation for $client_formatted on $installation_domain ($ip_address) performed.";
                    }
                else
                    {
                    $notification_case="notification_unknown_error";
                    $error_detected=1;
                    $error_reason="unknown error occurred";
                    }
                }
            else //this is existing installation, update its details
                {
                if (filter_var($client_id, FILTER_VALIDATE_INT)) //client_id is not null (so license_code is null), search by client_id (prepared statements has limitations when using where clauses with null values)
                    {
                    $stmt=mysqli_prepare($GLOBALS["mysqli"], "UPDATE apl_installations SET installation_date=?, installation_status=?, installation_hash=? WHERE client_id=? AND product_id=? AND installation_ip=? AND installation_domain=?");
                    if ($stmt)
                        {
                        mysqli_stmt_bind_param($stmt, "sisiiss", $installation_date, $installation_status, $installation_hash, $client_id, $product_id, $ip_address, $installation_domain);
                        $exec=mysqli_stmt_execute($stmt);
                    ...
                    {
                    $stmt=mysqli_prepare($GLOBALS["mysqli"], "UPDATE apl_installations SET installation_date=?, installation_status=?, installation_hash=? WHERE license_code=? AND product_id=? AND installation_ip=? AND installation_domain=?");
                    if ($stmt)
                        {
                        mysqli_stmt_bind_param($stmt, "sississ", $installation_date, $installation_status, $installation_hash, $license_code, $product_id, $ip_address, $installation_domain);
                        $exec=mysqli_stmt_execute($stmt);
                        $affected_rows=mysqli_stmt_affected_rows($stmt); if ($affected_rows>0) {$updated_records=$updated_records+$affected_rows;}
                        mysqli_stmt_close($stmt);
                        }
                ...
                if ($exec) //don't use $updated_records variable here because when installation_date variable is the same (script launched Nth time same day), $affected_rows will be 0
                    {
                    $action_success=1;
                    $report_text="Existing $product_title installation for $client_formatted on $installation_domain ($ip_address) updated.";
                    }
                else
                    {
                    $notification_case="notification_unknown_error";
        ...
        else //nobody knows what license status is... oh well, return unknown error
            {
            $notification_case="notification_unknown_error";
            $error_detected=1;
            $error_reason="unknown error occurred";
    ...
    if ($action_success==1) //everything OK (license_signature will also be displayed in this case)
        ...
        {
        $report_text="New $product_title installation for $client_formatted on $installation_domain ($ip_address) failed because of this reason: $error_reason."; //$notification_case is already created above in this case
        }
