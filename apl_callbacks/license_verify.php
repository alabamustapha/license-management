<?php
//used to check if license is active and add callback details to database during license check
...
if (filter_var($ip_address, FILTER_VALIDATE_IP) && in_array($user_agent, $SUPPORTED_BROWSERS_ARRAY) && FILTER_VAR($product_id, FILTER_VALIDATE_INT) && filter_var($root_url, FILTER_VALIDATE_URL, FILTER_FLAG_SCHEME_REQUIRED | FILTER_FLAG_HOST_REQUIRED) && $root_url==$refer && !empty($installation_hash) && $installation_hash==hash("sha256", $root_url.$client_email.$license_code) && !empty($license_signature))
    ...
    if ($error_detected!=1) //everything OK so far, do final check if license is really active
        ...
        if ($license_status==1) //license active, check if installation is stored in database
            ...
            if ($selected_installation_results_total>0) //installation exists
                {
                $action_success=1;
                $report_text="$product_title license verification for $client_formatted on $installation_domain ($ip_address) succeeded.";
                }
    ...
    if ($action_success==1) //everything OK (license_signature will also be displayed in this case)
        ...
        {
        $callback_status=0;
        $report_text="$product_title license verification for $client_formatted on $installation_domain ($ip_address) failed because of this reason: $error_reason."; //$notification_case is already created above in this case
        }


    $stmt=mysqli_prepare($GLOBALS["mysqli"], "INSERT IGNORE INTO apl_callbacks (client_id, license_code, product_id, callback_ip, callback_domain, callback_date, callback_status) VALUES (?, ?, ?, ?, ?, ?, ?)");
    if ($stmt)
        {
        $callback_domain=getRootUrl("$root_url/", 1, 1, 0, 1); //make url without scheme, www. and / at the end because this type of url is stored on server (add / at the end before processing because software stores root url without /)
        $callback_date=date("Y-m-d");

        mysqli_stmt_bind_param($stmt, "isisssi", $client_id, $license_code, $product_id, $ip_address, $callback_domain, $callback_date, $callback_status);
        $exec=mysqli_stmt_execute($stmt);
